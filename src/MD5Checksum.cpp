...
//////////////////////////////////////////////////////////////////////////////////////////
//
//?MD5ChecksumDefines.h????,Anthony 2010.8.11??????
//
/*****************************************************************************************
...
std::wstring CMD5Checksum::GetMD5(std::wstring strFilePath)
{
  ...
  FILE* File;
  char *path=WideChar2MultiByte(strFilePath.c_str());
  File=fopen(path, "rb");
...
std::wstring CMD5Checksum::Final()
{
  ...
  DWord2Byte(lpszMD5, m_lMD5, nMD5Size);
  ...
  for (int i=0; i < nMD5Size; i++) 
  {
    ...
    if (this->lpszMD5[i] == '\0')
	{
		//wcscpy_s(Str, sizeof(Str), L"00");//用这个会莫名其妙的运行错误,第二个参应该传递的是数组元素的数目128,而不是sizeofStr(256)
         wcscpy(Str, L"00");
		
	//swprintf_s(Str,128,L"00");//或者用这句
	}
    else if (this->lpszMD5[i] <= 15)
      swprintf(Str, 128, L"0%x", this->lpszMD5[i]);//第二个参数是Maximum number of characters to store，应该是sizeofStr(256）
    else
